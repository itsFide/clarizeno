...
const navItems = document.querySelectorAll('.page-nav__content-item');
  const sections = document.querySelectorAll('.content > section, .content > div[id]');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function safeAppendReply(parentId, replyData) {
      ...
      addReply(parentId, replyData.name, replyData.text, replyData.image, replyData.date, replyData.id);
    }
    ...
    Object.entries(config).forEach(([id, data]) => {
      ...
      if (replyBtn) replyBtn.addEventListener('click', () => openReplyForm(id, el));
    });
    ...
    /* ---------- ADD REPLY ---------- */
    function addReply(parentId, name, text, image, dateStr = null, forcedId = null) {
      const wrapper = document.querySelector(`#${parentId} .comments-item-reply-wrap`) ||
                      (() => {
      ...
      replyEl.innerHTML = `
        <div class="comments-item-info">
          <div class="comments-item__content">
            ...
            ${image ? `<div class="comments-img"><img src="${image}" style="max-width:200px;border-radius:8px;"></div>` : ''}
          </div>
        </div>
      `;
      wrapper.appendChild(replyEl);
  
      const storedReplies = JSON.parse(localStorage.getItem(`replies_${parentId}`) || '[]');
      if (!storedReplies.find(r => r.id === replyId)) {
        storedReplies.push({ id: replyId, name, text, image, date: finalDate });
        localStorage.setItem(`replies_${parentId}`, JSON.stringify(storedReplies));
      }
  
      bindLike(replyId);
      replyEl.querySelector('.comments-item__reply').addEventListener('click', () => openReplyForm(replyId, replyEl));
    }
    ...
    function bindLike(id) {
      ...
      const likeEl = el.querySelector('.comments-item__like p');
      const likeBtn = el.querySelector('.comments-item__like');
      let count = parseInt(localStorage.getItem(`likes_${id}`) || likeEl.textContent || 0);
      likeEl.textContent = count;
      const liked = localStorage.getItem(`liked_${id}`) === 'true';
      if (liked) likeBtn.classList.add('liked');
      likeBtn.addEventListener('click', () => {
    ...
    savedComments.forEach(c => renderMainComment(c.id, c.name, c.text, c.image, c.date));
    ...
    mainForm.querySelector('.comments-form-btn').addEventListener('click', e => {
      e.preventDefault();
      const inputs = mainForm.querySelectorAll('input[name="name"]');
      const name = inputs[0].value.trim();
      const email = inputs[1].value.trim();
      const text = mainForm.querySelector('textarea[name="comment"]').value.trim();
      if (!name || !email || !text) {
      ...
      if (!userComments.find(c => c.id === newId)) {
        userComments.push({ id: newId, name, text, image: uploadedMainImage, date: dateStr });
    ...
    function renderMainComment(id, name, text, image, dateStr) {
      ...
      bindLike(id);
      newEl.querySelector('.comments-item__reply').addEventListener('click', () => openReplyForm(id, newEl));
    }
